...
export function ScentMap(value) {
    switch (value) {
        case 'perfume':
            return 'پرفیوم';
        case 'eau-de-perfume':
...
function Products(props) {
    ...
    //sort products
    // const sortedProducts = sortProducts(productArray, queryParams.get('sort'));
    const sortedProducts = useFilterAndSort(productArray, queryParams.get("sort"), queryParams.get("filter"));
        ...
        <div className=" grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5">
            <div className="p-2  w-full  h-[350px] col-span-1 sm:col-span-1 md:col-span-1 lg:col-span-1 border border-gray-500 rounded-xl">
                ...
                <ul className="font-bold mb-5">
                    ...
                    <NavLink to={'men'}>آقایان : </NavLink>
                    {subCategoryArray.map((el, i) => {
                        if (el.category === 'men') {
                            return <li key={i + 1} >
                                <NavLink to={`${el.category}/${el.subCategory}`}>{ScentMap(el.subCategory)
                                }</NavLink>
                            </li>
                        }
                    })}
                ...
                <ul className="font-bold mt-5">
                    {isLoadingSubCategory && <Spinner />}
                    <NavLink to={'women'}>خانم‌ها : </NavLink>
                    {subCategoryArray.map((el, i) => {
            ...
            <div className=" p-2 w-full sm:col-span-2 md:col-span-3 lg:col-span-4 xl:col-span-4 border border-gray-500 rounded-xl">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4">
                    ...
                    {sortedProducts.slice(visitedPosts, currentPage * postsPerPage).map((el, i) => <MyCard key={i + 1} imgSrc={`http://localhost:3002${el.image}`} name={el.name} description={el.description} brand={el.brand} price={Number(el.price)} id={el.id} category={el.category} subCategory={el.subCategory} linkPath={`${el.category}/${el.subCategory}/${el.id}`} />)}
